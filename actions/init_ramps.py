...
#                    comment="evaporation ramp")
    ...
    action_list.add("B comp x func", lib_ramp.FunctionRamp,
                    categories=["func"],
                    parameters=dict(act_name="B comp x", act_var_name="value", act_parameters={}),
    ...
    action_list.add("Delta 1 current func", lib_ramp.FunctionRamp,
                    categories=["func"],
                    parameters=dict(act_name="Delta 1 Current", act_var_name="value", act_parameters={}),
                    variables=dict(start_t=0, stop_t=100, n_points=100,
                                   func="amp*sin(2*pi*freq*t) + offset", func_args="amp=5, freq=200, offs=50"),
                    var_formats=dict(start_t="%.4f", stop_t="%.4f", n_points="%d", func="%s", func_args="%s"),
                    comment="time")  
...
# Time repetition ramps:
    ...
    action_list.add("Single_frame_imaging ramp", lib_ramp.LinearRamp,
                    categories=["ramps"],
                    parameters=dict(act_name="Single_frame_imaging"),
                    variables=dict(start_x=1, stop_x=0, start_t=0, stop_t=100, n_points=100),
                    var_formats=dict(start_x="%.3f", stop_x="%.3f", start_t="%.4f", stop_t="%.4f", n_points="%d"),
                    comment="time")
