...
prg_version = "0.7"
def program(prg, cmd):
    prg.add(-13150, "B grad comp OFF", functions=dict(time=lambda x: x+cmd.get_var('tof2')+500))
    prg.add(0, "Config Levitation dipole", enable=False)
    prg.add(0, "Config Field OFF.sub", functions=dict(time=lambda x: x+cmd.get_var('tof2')-cmd.get_var('t_cfo')), enable=False)
