...
def program(prg, cmd):
    ...
    prg.add(159999990, "Scope 1 Trigger ON", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('evap3_time')+cmd.get_var('hold_time')+cmd.get_var('hold_time_after')+cmd.get_var('tof')-0.009))
    prg.add(159999990, "Scope 1 Trigger OFF", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('evap3_time')+cmd.get_var('hold_time')+cmd.get_var('hold_time_after')+cmd.get_var('tof')+.15))
    ...
    prg.add(160000000, "Green Light AOM amp", 0, functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('evap3_time')+cmd.get_var('hold_time')+cmd.get_var('green_time')+cmd.get_var('hold_time_after')+cmd.get_var('tof')), enable=False)
    prg.add(160000000, "Setup_imaging", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('evap3_time')+cmd.get_var('hold_time')+cmd.get_var('hold_time_after')+cmd.get_var('tof')))
    ...
    prg.add(160000000, "BEC_imaging_z_vertical", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('evap3_time')+cmd.get_var('hold_time')+cmd.get_var('hold_time_after')+cmd.get_var('tof')))
    prg.add(160000011, "Config Field OFF.sub", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('evap3_time')+cmd.get_var('hold_time')+cmd.get_var('hold_time_after')))
...
def commands(cmd):
    import numpy as np
    iters = np.arange(-0.18, -0.11, 0.01)
    j = 0
    while(cmd.running):
        print('\n-------o-------')
        Bx_bottom = iters[j]
