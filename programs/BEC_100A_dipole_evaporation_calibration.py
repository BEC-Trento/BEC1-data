...
def program(prg, cmd):
    ...
    prg.add(240002468, "Scope 1 Trigger Pulse", polarity=1, pulse_t=1.00000, functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('end_of_ODT_transfer')+cmd.get_var('dipole_hold_after')-0.001))
    prg.add(240002468, "Pulse uw with RF double", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('end_of_ODT_transfer')+cmd.get_var('dipole_hold_after')))
    ...
    prg.add(240002599, "Dipole trap xy STANDBY", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('end_of_ODT_transfer')))
    prg.add(240002620, "Setup_imaging", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('end_of_ODT_transfer')+cmd.get_var('dipole_mix_time')))
    prg.add(240002720, "BEC_imaging_field_OFF", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('Quad_rampdown2_time')+cmd.get_var('magnetic_trap_hold')+cmd.get_var('dipole_hold_time')+cmd.get_var('dipole_evap_time')+cmd.get_var('dipole_hold2_time')+cmd.get_var('dipole_mix_time')+cmd.get_var('dipole_hold3_time')+1e-3*cmd.get_var('marconi1_pulsetime')+1e-3*cmd.get_var('marconi1_pulsetime2')+cmd.get_var('dipole_evap2_time')+cmd.get_var('dipole_hold3_time')))
    prg.add(240015620, "Imaging_situ_tof", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('end_of_ODT_transfer')+cmd.get_var('dipole_mix_time')+cmd.get_var('grad_ramp_time')+cmd.get_var('grad_ramp_time2')*2+cmd.get_var('dipole_hold3_time')+cmd.get_var('dipole_hold4_time')), enable=False)
    ...
    prg.add(248003420, "DarkSpotMOT_19.sub", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('end_of_ODT_transfer')+cmd.get_var('dipole_mix_time')+cmd.get_var('grad_ramp_time')+cmd.get_var('grad_ramp_time2')*2+cmd.get_var('dipole_hold3_time')+cmd.get_var('dipole_hold4_time')+cmd.get_var('tof2')), enable=False)
    return prg
def commands(cmd):
    import numpy as np
