...
def program(prg, cmd):
    ...
    prg.add(230052100, "Evaporation freq", 0, functions=dict(frequency=lambda x: (cmd.get_var('evap2_fend')+cmd.get_var('rf_shield_offset'))*1e6, time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('evap3_time')+cmd.get_var('sync_time')+cmd.get_var('evap4_time')+cmd.get_var('evap5_time')+0.032))
