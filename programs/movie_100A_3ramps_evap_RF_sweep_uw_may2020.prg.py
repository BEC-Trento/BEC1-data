...
def program(prg, cmd):
    ...
    prg.add(230010440, "Scope 1 Trigger Pulse", polarity=1, pulse_t=0.01000, functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('t_movie_start') - 0.0348))
    ...
    prg.add(230010440, "Rf Sweep Siglent", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('evap3_time')+cmd.get_var('t_sweep_start')), enable=False)
    prg.add(230010440, "Setup_imaging", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('t_movie_start')-100.124))
    ...
    prg.add(230010440, "Config Levitation", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time') +cmd.get_var('evap3_time')+cmd.get_var('t_sweep_start')+cmd.get_var('hold_time')), enable=False)
    prg.add(230010450, "Config Field OFF.sub", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time') +cmd.get_var('evap3_time')+cmd.get_var('t_sweep_start')+cmd.get_var('hold_time')+cmd.get_var('tof2') -1), enable=False)
    prg.add(230010450, "BEC_imaging", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time') +cmd.get_var('evap3_time') +cmd.get_var('t_sweep_start')+cmd.get_var('hold_time')+cmd.get_var('tof2')), enable=False)
