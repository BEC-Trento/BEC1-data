...
def program(prg, cmd):
    ...
    prg.add(1000000, "open_probe_z", enable=False)
    ...
    prg.add(230000000, "B comp x func", start_t=0.0000, func_args="amp=5, freq=100, offset=1", n_points=100, func="amp*sin(2*pi*freq*t) + offset", stop_t=100.0000, functions=dict(func_args=lambda x: 'amp={}, freq={}, offset={}'.format(cmd.get_var('mod_amp'), cmd.get_var('mod_freq'),cmd.get_var('Bx_GM')+cmd.get_var('Bx_bottom')), stop_t=lambda x: cmd.get_var('mod_time'), n_points=lambda x: 1e-3*cmd.get_var('mod_time') * cmd.get_var('mod_freq') * 10, time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('evap3_time')+cmd.get_var('hold_time')))
    prg.add(230000000, "B comp x", 0.0, functions=dict(value=lambda x: cmd.get_var('Bx_GM')+cmd.get_var('Bx_bottom'), time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('evap3_time')+cmd.get_var('hold_time')+cmd.get_var('mod_time') + 0.1))
    prg.add(230000000, "Delta 1 current func", start_t=0.0000, func_args="amp=5, freq=200, offset=50", n_points=100, func="amp*sin(2*pi*freq*t) + offset", stop_t=100.0000, functions=dict(func_args=lambda x: 'amp={}, freq={}, offset={}'.format(cmd.get_var('mod_amp'), cmd.get_var('mod_freq'),cmd.get_var('Quad_rampdown2_current')), stop_t=lambda x: cmd.get_var('mod_time'), time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('evap3_time')+cmd.get_var('hold_time'), n_points=lambda x: 1e-3*cmd.get_var('mod_time') * cmd.get_var('mod_freq') * 10), enable=False)
    ...
    prg.add(240000000, "open_probe_z", enable=False)
    prg.add(250000000, "open_probe_z", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('evap3_time')+cmd.get_var('hold_time')+cmd.get_var('mod_time')+cmd.get_var('hold_time_2')+cmd.get_var('tof')), enable=False)
    return prg
def commands(cmd):
    import numpy as np
    iters = np.arange(80, 110, 2)
