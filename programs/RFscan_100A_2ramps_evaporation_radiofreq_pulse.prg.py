...
def program(prg, cmd):
    ...
    prg.add(190005010, "Config Levitation", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+  cmd.get_var('rf_pulse_time')))
    ...
    prg.add(195005010, "Pulse uw OFF", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time') + cmd.get_var('rf_pulse_time') +cmd.get_var('tof')), enable=False)
...
def commands(cmd):
    import numpy as np
    rf_freq1_arr, repeat_arr = np.mgrid[1.02:1.18:0.01, 0:1:1, ]
    iters = list(zip(rf_freq1_arr.ravel(), repeat_arr.ravel()))
    np.random.shuffle(iters)
    j = 0
    while(cmd.running):
        print('\n-------o-------')
        rf_freq1, repeat = iters[j]
        cmd.set_var('rf_freq1', rf_freq1)
        cmd.set_var('repeat', repeat)
        print('\n')
        print('Run #%d/%d, with variables:\nrf_freq1 = %g\nrepeat = %g\n'%(j+1, len(iters), rf_freq1, repeat))
