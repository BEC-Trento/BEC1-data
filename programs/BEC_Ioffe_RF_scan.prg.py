...
def program(prg, cmd):
    ...
    prg.add(160000000, "Pulse uw ON", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('hold_time')), enable=False)
    prg.add(160000000, "Pulse uw OFF", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('hold_time')+1e-3*cmd.get_var('marconi1_pulsetime')), enable=False)
    prg.add(160000000, "RF pulse ON", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('hold_time')))
    prg.add(160000000, "RF pulse OFF", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('hold_time')+1e-3*cmd.get_var('RF_pulsetime')))
    prg.add(160000000, "Single_frame_imaging", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('hold_time')+cmd.get_var('hold_time_2')), enable=False)
    prg.add(160000000, "Config Field OFF.sub", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('hold_time')+cmd.get_var('tof')))
    prg.add(160010000, "BEC_imaging_ready", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('hold_time')+cmd.get_var('tof')))
    ...
    prg.add(160065000, "RF pulse amp", 0, functions=dict(amplitude=lambda x: cmd.get_var('RF_amp')))
    prg.add(160066000, "RF pulse freq", 0.0000, functions=dict(frequency=lambda x: cmd.get_var('RF_freq')))
    prg.add(160070000, "Mirrors Imaging")
    prg.add(165020000, "Ramp_bias_down.sub")
    prg.add(170010000, "RF pulse amp", 0, functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('hold_time')+cmd.get_var('tof')))
