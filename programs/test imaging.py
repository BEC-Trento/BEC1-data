...
def program(prg, cmd):
    ...
    prg.add(10000000, "Scope 1 Trigger Pulse", polarity=1, pulse_t=0.01448, functions=dict(time=lambda x: x-0.00154), enable=False)
    ...
    prg.add(10740000, "BEC_imaging", enable=False)
    prg.add(10740000, "Imaging_single_shot_reconstruction")
    prg.add(11740000, "BEC_imaging_double", enable=False)
    prg.add(11940001, "Setup_tof_imaging", enable=False)
    prg.add(12740000, "BEC_imaging", enable=False)
    prg.add(36940001, "open_probe")
    return prg
def commands(cmd):
    import numpy as np
