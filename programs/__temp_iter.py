...
def program(prg, cmd):
    ...
    prg.add(388150000, "Rabi oscillation 0 to 1-1 New Antena", enable=False)
    prg.add(388150000, "Rabi oscillation 0 to 1-1 new quantization New Antena", enable=False)
    prg.add(388150000, "Rabi preparation")
    prg.add(392200000, "B comp y ramp", start_t=0, stop_x=0, n_points=150, start_x=1500, stop_t=80, enable=False)
    prg.add(392210000, "ReleConfigBackForGravityComp")
    prg.add(392570000, "Rabi pulse New Antena")
    prg.add(392606000, "Dipole Trap y DAC V", 0.0000)
    prg.add(392606400, "Dipole Trap x DAC V", 0.0000)
    prg.add(392638800, "Picture - Field off at 0ms -SternGerlach_Levit 10ms.sub")
    prg.add(392638800, "Picture - Field off at 0ms - Levit 50ms.sub", enable=False)
    prg.add(392638800, "Picture -DipoleLoad-SternGerlach_Levit 10ms.sub", enable=False)
    prg.add(392638800, "Picture - Field off at 0ms - Levit 20ms.sub", enable=False)
    prg.add(392818800, "Picture - DipoleLoad - Levit 50ms.sub", enable=False)
    prg.add(393118800, "Picture NaK.sub", enable=False)
    prg.add(397118800, "Preparation mixture 1 -1 spin exchange", enable=False)
    prg.add(397119800, "Rabi oscillation 0 to 1-1", enable=False)
    prg.add(403418800, "excite pinch", enable=False)
    prg.add(412028800, "spin cleaning pinch", enable=False)
    prg.add(415292200, "Bias field initialization")
    prg.add(423795300, "Pulse uw OFF", enable=False)
    prg.add(424277400, "Set MOT NaK.sub")
    prg.add(424777400, "Dark Spot MOT load.sub")
    prg.add(424877400, "Config MOT.sub")
    return prg
