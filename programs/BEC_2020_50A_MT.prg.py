...
def program(prg, cmd):
    ...
    prg.add(250000000, "Delta 1 Current ramp", start_t=0.0000, stop_x=50.000, n_points=200, start_x=200.000, stop_t=3000.0000)
    prg.add(295000000, "Config Field OFF.sub")
    prg.add(295000000, "Scope 1 Trigger Pulse", polarity=1, pulse_t=0.01000, functions=dict(time=lambda x: x + cmd.get_var('tof') - 0.00123))
    prg.add(295000000, "Setup_imaging", functions=dict(time=lambda x: x + cmd.get_var('tof')))
    prg.add(295000000, "BEC_imaging", functions=dict(time=lambda x: x + cmd.get_var('tof')))
    prg.add(305000000, "DarkSpotMOT_19.sub", enable=False)
    prg.add(305300000, "open_probe", enable=False)
    return prg
def commands(cmd):
    import numpy as np
