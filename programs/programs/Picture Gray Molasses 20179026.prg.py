...
def program(prg, cmd):
    ...
    prg.add(112297155, "Gray Molasses Pulse 2017", enable=False)
    prg.add(112297155, "Gray Molasses 2017")
    prg.add(112357155, "empty.sub", enable=False)
    prg.add(112359005, "Picture close.sub", functions=dict(time=lambda x: x + cmd.get_var('dt')), enable=False)
    prg.add(121165565, "TTL2-12 OFF")
    prg.add(121272745, "Set MOT NaK.sub")
    prg.add(121772745, "Dark Spot MOT load.sub")
    prg.add(121872745, "Config MOT.sub")
    return prg
def commands(cmd):
    import numpy as np
