...
def program(prg, cmd):
    ...
    prg.add(813335250, "Pulse Trig Extra Hamamatsu", polarity=1, pulse_t=0.10000)
    ...
    prg.add(814335150, "TTL Repumper MOT ON", enable=False)
    prg.add(814335150, "TTL Repumper MOT OFF", enable=False)
    prg.add(814335250, "Picture NaK Ready no Rep.sub")
    prg.add(814335250, "Picture NaK Ready no Rep Trig2.sub", enable=False)
    prg.add(814635250, "Config Field OFF.sub")
    prg.add(814645250, "Picture NaK Ready.sub", enable=False)
    prg.add(822567390, "TTL2-12 OFF")
    prg.add(822667390, "Pulse uw OFF")
    prg.add(822668650, "Set MOT NaK.sub")
    prg.add(823167390, "Dark Spot MOT load.sub")
    prg.add(823267390, "Config MOT.sub")
    return prg
def commands(cmd):
    import numpy as np
