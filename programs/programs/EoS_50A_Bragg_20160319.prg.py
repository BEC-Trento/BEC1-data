...
def program(prg, cmd):
    ...
    prg.add(680270000, "Picture - In Situ.sub", functions=dict(time=lambda x: x + cmd.get_var("t"), funct_enable=False))
    prg.add(680270000, "Config Field OFF.sub", enable=False)
    prg.add(680270000, "Picture - Field off at 0ms - Levit 50ms.sub", enable=False)
    prg.add(680370000, "Picture NaK.sub", enable=False)
    prg.add(680370000, "Picture - Field off at 0ms - Levit 150ms.sub", enable=False)
    prg.add(690920000, "Set MOT NaK.sub")
    prg.add(691420000, "Dark Spot MOT load.sub")
    prg.add(691520000, "Config MOT.sub")
    return prg
def commands(cmd):
