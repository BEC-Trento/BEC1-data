...
def program(prg, cmd):
    ...
    prg.add(119010400, "B comp x", 350.0)
    ...
    prg.add(378315000, "B comp x ramp", start_t=0, stop_x=0, n_points=300, start_x=350, stop_t=250)
    ...
    prg.add(392800000, "Preparation mixture 1 -1 spin exchange", enable=False)
    prg.add(392801000, "Rabi oscillation 0 to 1-1", enable=False)
    prg.add(392801000, "Rabi oscillation 0 to 1-1 New Antena")
    prg.add(393400000, "Dipole Trap y DAC V", 0.0000)
    prg.add(393400400, "Dipole Trap x DAC V", 0.0000)
    prg.add(393402400, "Picture - Field off at 0ms -SternGerlach_Levit 10ms.sub")
    prg.add(398800000, "excite pinch", enable=False)
    prg.add(400673400, "Picture NaK.sub", enable=False)
    prg.add(410673400, "Bias field initialization")
    prg.add(419176500, "Pulse uw OFF", enable=False)
    prg.add(419658600, "Set MOT NaK.sub")
    prg.add(420158600, "Dark Spot MOT load.sub")
    prg.add(420258600, "Config MOT.sub")
    return prg
def commands(cmd):
