...
def program(prg, cmd):
    ...
    prg.add(746300000, "Picture - Field off at 0ms - Levit 100ms.sub", enable=False)
    ...
    prg.add(746300000, "Picture Levit 20170908.sub")
    prg.add(746300000, "Config Field OFF.sub", enable=False)
    prg.add(746300135, "TTL2-12 ON")
    prg.add(746310000, "Picture NaK.sub", enable=False)
    prg.add(748100040, "TTL2-12 OFF")
    prg.add(754100040, "Bx Grad OFF")
    prg.add(754200040, "Set MOT NaK.sub")
    prg.add(754700040, "Dark Spot MOT load.sub")
    prg.add(754800040, "Config MOT.sub")
    return prg
def commands(cmd):
    import numpy as np
