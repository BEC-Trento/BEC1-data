...
def program(prg, cmd):
    ...
    prg.add(459371200, "Dipole Trap y ramp", start_t=0.0000, stop_x=4.000, n_points=150, start_x=2.000, stop_t=200.0000, enable=False)
    prg.add(467381100, "Decompress Current 200-50", start_t=0.0000, stop_x=0.000, n_points=300, start_x=50.000, stop_t=700.0000)
    prg.add(474391100, "Dipole Trap x ramp", start_t=0.0000, stop_x=1.500, n_points=150, start_x=1.000, stop_t=50.0000)
    prg.add(474391200, "Dipole Trap y ramp", start_t=0.0000, stop_x=4.000, n_points=150, start_x=2.000, stop_t=50.0000)
    ...
    prg.add(488285699, "Dipole Trap x DAC V", 0.0000, functions=dict(time=lambda x: x+cmd.get_var('dt'), funct_enable=False))
    prg.add(488285799, "Dipole Trap y DAC V", 0.0000, functions=dict(time=lambda x: x+cmd.get_var('dt'), funct_enable=False))
    prg.add(488285899, "TTL2-10 OFF", functions=dict(time=lambda x: x+cmd.get_var('dt'), funct_enable=False))
    prg.add(488285999, "Picture NaK.sub", functions=dict(time=lambda x: x+cmd.get_var('dt'), funct_enable=False), enable=False)
    prg.add(488285999, "Picture - Field off at 0ms -SternGerlach_Levit 10ms.sub", enable=False)
    prg.add(488285999, "Picture - Field off at 0ms -SternGerlach_Ele.sub", functions=dict(time=lambda x: x+cmd.get_var('dt'), funct_enable=False))
    prg.add(488285999, "Picture - Field off at 0ms -SternGerlach_ShortElena.sub", enable=False)
    prg.add(488285999, "Picture - Field off at 0ms - Levit 50ms.sub", enable=False)
    prg.add(488286099, "Picture - Field off at 0ms -SternGerlach_ShortElena2017.sub", functions=dict(time=lambda x: x+cmd.get_var('dt'), funct_enable=False), enable=False)
    prg.add(488456099, "Picture NaK.sub", enable=False)
    prg.add(488456099, "Picture NaK no Rep.sub", enable=False)
    prg.add(494466099, "Bx Grad OFF", functions=dict(time=lambda x: x+cmd.get_var('dt'), funct_enable=False))
    prg.add(494466199, "TTL2 5 OFF", enable=False)
    prg.add(506239598, "Bias field initialization", functions=dict(time=lambda x: x+cmd.get_var('dt'), funct_enable=False))
    prg.add(507090296, "Pulse uw OFF", functions=dict(time=lambda x: x+cmd.get_var('dt'), funct_enable=False))
    prg.add(515324797, "Set MOT NaK.sub", functions=dict(time=lambda x: x+cmd.get_var('dt'), funct_enable=False))
    prg.add(515824797, "Dark Spot MOT load.sub", functions=dict(time=lambda x: x+cmd.get_var('dt'), funct_enable=False))
    prg.add(515924797, "Config MOT.sub", functions=dict(time=lambda x: x+cmd.get_var('dt'), funct_enable=False))
    return prg
def commands(cmd):
    import numpy as np
    iters = np.arange(0, 40, 3)
    j = 0
    while(cmd.running):
        dt1 = iters[j]
        cmd.set_var('dt', dt1)
