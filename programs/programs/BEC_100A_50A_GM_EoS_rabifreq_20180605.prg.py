...
def program(prg, cmd):
    ...
    prg.add(199791645, "B comp x", 680.0)
    ...
    prg.add(803845000, "Pulse uw", polarity=1, pulse_t=0.07000, functions=dict(pulse_t=lambda x: 1e-3*cmd.get_var('tau'), funct_enable=False))
    prg.add(803845100, "Picture NaK Ready no Rep Trig2.sub")
    prg.add(803845200, "Picture Levit 2017 - setup")
    prg.add(803846200, "Delta 1 Current", 13.500)
    prg.add(804836200, "IGBT 4 Open")
    prg.add(804846200, "Picture NaK Ready.sub")
    prg.add(812768340, "TTL2-12 OFF")
    prg.add(812868340, "Pulse uw OFF")
    prg.add(812869600, "Set MOT NaK.sub")
    prg.add(813368340, "Dark Spot MOT load.sub")
    prg.add(813468340, "Config MOT.sub")
    return prg
def commands(cmd):
    import numpy as np
