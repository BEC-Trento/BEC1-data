...
def program(prg, cmd):
    ...
    prg.add(766905000, "B comp y ramp", start_t=0, stop_x=2.5, n_points=100, start_x=0, stop_t=1000, functions=dict(stop_x=lambda x: cmd.get_var('by'), funct_enable=False))
    ...
    prg.add(781921000, "Picture Levit 2017 - 50ms", enable=False)
    prg.add(781922000, "Delta 1 Voltage", 0.0000)
    prg.add(781922750, "IGBT 2 pinch+comp", -10.0000)
    prg.add(781923500, "IGBT 5 Open")
    prg.add(781935500, "Pulse uw", polarity=1, pulse_t=0.00720)
    prg.add(781936500, "Picture NaK Ready no Rep Trig2.sub")
    prg.add(781936500, "Picture NaK Ready Trig 2.sub", enable=False)
    prg.add(781936500, "Picture NaK Ready.sub", enable=False)
    prg.add(781939000, "Picture Levit CFO 2018 - 10ms")
    prg.add(789957640, "TTL2-12 OFF")
    prg.add(790057640, "Pulse uw OFF")
    prg.add(790058900, "Set MOT NaK.sub")
    prg.add(790557640, "Dark Spot MOT load.sub")
    prg.add(790657640, "Config MOT.sub")
    return prg
def commands(cmd):
    import numpy as np
