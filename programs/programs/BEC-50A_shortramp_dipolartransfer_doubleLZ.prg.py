...
def program(prg, cmd):
    ...
    prg.add(378315000, "B comp x ramp", start_t=0, stop_x=0, n_points=300, start_x=325, stop_t=250)
    prg.add(380820000, "Decompress Current 200-50", start_t=0.0000, stop_x=0.000, n_points=300, start_x=50.000, stop_t=200.0000)
    prg.add(382830000, "uniform B field x Rele IGBT config")
    prg.add(384330000, "Decompress Current 200-50", start_t=0.0000, stop_x=108.000, n_points=300, start_x=0.000, stop_t=250.0000)
    prg.add(386930000, "Decompress Current 200-50", start_t=0.0000, stop_x=109.000, n_points=300, start_x=108.000, stop_t=25.0000)
    prg.add(386931000, "RF mag sweep 0 level")
    prg.add(387280000, "Decompress Current 200-50", start_t=0.0000, stop_x=0.000, n_points=300, start_x=109.000, stop_t=250.0000)
    ...
    prg.add(392800000, "RF mag sweep external level", enable=False)
    prg.add(393120000, "Dipole Trap y DAC V", 0.0000)
    prg.add(393121000, "Dipole Trap x DAC V", 0.0000)
    prg.add(393126000, "Picture - Field off at 0ms -SternGerlach_Levit 10ms.sub")
    prg.add(393297000, "Picture NaK.sub", enable=False)
    prg.add(403315200, "Set MOT NaK.sub")
    prg.add(403632500, "Config Field OFF.sub", enable=False)
    prg.add(403815200, "Dark Spot MOT load.sub")
    prg.add(403915200, "Config MOT.sub")
    return prg
def commands(cmd):
