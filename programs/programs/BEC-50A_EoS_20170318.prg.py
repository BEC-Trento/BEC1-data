...
def program(prg, cmd):
    ...
    prg.add(781930000, "Pulse uw", polarity=1, pulse_t=0.00100, enable=False)
    prg.add(781931000, "Picture NaK Ready no Rep Trig2.sub", enable=False)
    prg.add(781931000, "Picture NaK Ready Trig 2.sub", enable=False)
    prg.add(781933500, "Picture Levit 2017 - setup")
    prg.add(781935500, "Delta 1 Current", 13.500)
    prg.add(782925500, "IGBT 4 Open")
    prg.add(782935500, "Picture NaK Ready.sub")
    prg.add(790857640, "TTL2-12 OFF")
    prg.add(790957640, "Pulse uw OFF")
    prg.add(790958900, "Set MOT NaK.sub")
    prg.add(791457640, "Dark Spot MOT load.sub")
    prg.add(791557640, "Config MOT.sub")
    return prg
def commands(cmd):
    import numpy as np
