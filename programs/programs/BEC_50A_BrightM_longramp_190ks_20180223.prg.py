...
def program(prg, cmd):
    ...
    prg.add(209057960, "B comp x", 890.0)
    ...
    prg.add(754300000, "TTL2-12 OFF")
    prg.add(759300000, "empty.sub")
    prg.add(759300000, "Picture Levit 2017 - 20ms", enable=False)
    prg.add(759300000, "Picture Levit 2017 - 50ms")
    prg.add(759300000, "Picture Levit 2017 - 80ms", enable=False)
    prg.add(759300000, "Picture Levit 2017 - 100ms", enable=False)
    prg.add(759300000, "Picture Levit 2017 - 120ms", enable=False)
    prg.add(759300000, "Picture Levit 2017 - 150ms", enable=False)
    prg.add(759300000, "Picture Levit 2017 - 180ms", enable=False)
    prg.add(759300000, "Config Field OFF.sub", functions=dict(time=lambda x: x+cmd.get_var('dt'), funct_enable=False), enable=False)
    prg.add(759310000, "Picture NaK.sub", enable=False)
    prg.add(767100040, "Bx Grad OFF")
    prg.add(767200040, "Set MOT NaK.sub")
    prg.add(767700040, "Dark Spot MOT load.sub")
    prg.add(767800040, "Config MOT.sub")
    return prg
def commands(cmd):
    import numpy as np
