...
def program(prg, cmd):
    ...
    prg.add(474366000, "Trigger LZ ON", enable=False)
    prg.add(474466000, "Trigger LZ OFF", enable=False)
    prg.add(474470000, "Config Field OFF.sub", enable=False)
    prg.add(474480000, "Config Feshbach.sub", enable=False)
    prg.add(474680000, "Feshbach ramp 20ms 0-50A.sub", enable=False)
    prg.add(474780000, "Delta 1 Current", 50.000000, enable=False)
    prg.add(474780009, "Delta 2 Voltage", 15.000000, enable=False)
    prg.add(475900000, "Feshbach ramp 10ms 50-0A.sub", enable=False)
    prg.add(477450000, "Trigger uw sweep ON")
    prg.add(477451000, "Trigger uw amp ON")
    prg.add(477550000, "Trigger uw sweep OFF")
    prg.add(477551000, "Trigger uw amp OFF")
    prg.add(477569000, "Dipole Trap x (+) Amp", 1.000000)
    prg.add(477569400, "Dipole Trap y (-) Amp", 1.000000)
    prg.add(477570000, "Config Levitation F1+1.sub")
    prg.add(477720000, "Config Field OFF.sub")
    prg.add(477726000, "Picture NaK.sub")
    prg.add(480000000, "Shutter OT x Close")
