...
def program(prg, cmd):
    ...
    prg.add(592640000, "Config Field OFF.sub", enable=False)
    prg.add(592650000, "Picture NaK.sub", enable=False)
    ...
    prg.add(624300000, "Picture Levit 2017 - 15ms", enable=False)
    prg.add(624300000, "Picture Levit 2017 - 50ms", enable=False)
    prg.add(624300000, "Picture Levit 2017 - 150ms")
    prg.add(624300000, "Picture Levit 2017 - 180ms", enable=False)
    prg.add(624300000, "Picture Levit 2017 - 200ms", enable=False)
    prg.add(624310000, "Picture NaK.sub", enable=False)
    prg.add(624310000, "empty.sub", enable=False)
    prg.add(632310000, "Set MOT NaK.sub")
    prg.add(632810000, "Dark Spot MOT load.sub")
    prg.add(632910000, "Config MOT.sub")
    return prg
def commands(cmd):
    import numpy as np
