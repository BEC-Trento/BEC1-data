...
def program(prg, cmd):
    ...
    prg.add(392603000, "Pulse uw OFF")
    prg.add(392609000, "Dipole Trap y DAC V", 0.0000)
    prg.add(392609400, "Dipole Trap x DAC V", 0.0000)
    prg.add(392641800, "Picture - Field off at 0ms -SternGerlach_Levit 10ms.sub")
    prg.add(392641800, "Picture - Field off at 0ms - Levit 50ms.sub", enable=False)
    prg.add(392641800, "Picture -DipoleLoad-SternGerlach_Levit 10ms.sub", enable=False)
    prg.add(392641800, "Picture - Field off at 0ms - Levit 20ms.sub", enable=False)
    prg.add(392821800, "Picture - DipoleLoad - Levit 50ms.sub", enable=False)
    prg.add(393121800, "Picture NaK.sub", enable=False)
    prg.add(397121800, "Preparation mixture 1 -1 spin exchange", enable=False)
    prg.add(397122800, "Rabi oscillation 0 to 1-1", enable=False)
    prg.add(403421800, "excite pinch", enable=False)
    prg.add(412031800, "spin cleaning pinch", enable=False)
    prg.add(415295200, "Bias field initialization")
    prg.add(424280400, "Set MOT NaK.sub")
    prg.add(424780400, "Dark Spot MOT load.sub")
    prg.add(424880400, "Config MOT.sub")
    return prg
def commands(cmd):
