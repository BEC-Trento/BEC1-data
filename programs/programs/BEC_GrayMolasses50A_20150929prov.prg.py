...
def program(prg, cmd):
    ...
    prg.add(139998020, "B comp x", 102.0)
    prg.add(145000020, "Evaporation Ramp.sub")
    prg.add(445000020, "Decompress Current 200-50", start_t=0.0000, stop_x=50.000, n_points=150, start_x=200.000, stop_t=600.0000, enable=False)
    prg.add(445010020, "Decompress Voltage 200-50", start_t=0.0000, stop_x=0.000, n_points=150, start_x=30.000, stop_t=600.0000, enable=False)
    prg.add(455010000, "Picture - Field off at 0ms - Levit 20ms.sub", enable=False)
    prg.add(455010000, "Picture - Field off at 0ms - Levit 5ms.sub", enable=False)
    prg.add(632000000, "Decompress Current MT.sub")
    prg.add(632010000, "Decompress Voltage MT.sub")
    prg.add(637010000, "Config Field OFF.sub")
    prg.add(637110000, "Picture NaK.sub")
    prg.add(794013982, "Set MOT NaK.sub")
    prg.add(794513982, "Config MOT.sub")
    prg.add(794613982, "Dark Spot MOT load.sub")
    return prg
