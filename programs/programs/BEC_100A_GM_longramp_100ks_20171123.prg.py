...
def program(prg, cmd):
    ...
    prg.add(826231560, "Pulse TTL2-12", polarity=1, pulse_t=0.10000)
    prg.add(826232260, "TTL0-13 broken OFF", enable=False)
    prg.add(826232260, "Picture Levit 2017 - 30ms", enable=False)
    prg.add(826232260, "Picture Levit 2017 - 50ms", enable=False)
    prg.add(826232260, "Picture Levit 2017 - 100ms")
    prg.add(826237260, "B comp x", 750.0)
    prg.add(826237260, "Picture NaK short delay.sub", enable=False)
    prg.add(826267260, "Config Field OFF.sub", enable=False)
    prg.add(826272260, "Picture NaK.sub", functions=dict(time=lambda x: x + cmd.get_var('tof'), funct_enable=False), enable=False)
    prg.add(826302260, "Config Field OFF.sub", enable=False)
    prg.add(842042685, "Set MOT NaK.sub")
    prg.add(842542685, "Dark Spot MOT load.sub")
    prg.add(842642685, "Config MOT.sub")
    return prg
def commands(cmd):
    import numpy as np
