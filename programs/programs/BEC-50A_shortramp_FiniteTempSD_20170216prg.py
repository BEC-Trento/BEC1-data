...
def program(prg, cmd):
    ...
    prg.add(21069900, "B comp x", 1040.0)
    ...
    prg.add(282802050, "Dipole Trap y ramp", start_t=0.0000, stop_x=4.500, n_points=200, start_x=2.000, stop_t=100.0000)
    prg.add(282812050, "Dipole Trap x ramp", start_t=0.0000, stop_x=4.500, n_points=100, start_x=2.000, stop_t=100.0000)
    prg.add(284812050, "heating dipole", enable=False)
    prg.add(289812050, "LZ -1 to 0")
    prg.add(296312050, "spin cleaning pinch", enable=False)
    prg.add(299812050, "Rabi preparation 2")
    prg.add(304182100, "ReleConfigBackForGravityComp")
    prg.add(304682100, "Rabi pulse New Antena")
    prg.add(304686100, "Dipole Trap x DAC V", 0.0000)
    prg.add(304689100, "Dipole Trap y DAC V", -0.1000)
    prg.add(304759100, "Picture NaK.sub")
    prg.add(304759100, "Picture - Field off at 0ms - Levit 100ms.sub", enable=False)
    prg.add(304759100, "Picture - Field off at 0ms - Levit 20ms.sub", enable=False)
    prg.add(304759100, "Picture - Field off at 0ms - Levit 10ms.sub", enable=False)
    prg.add(304759100, "Picture - Field off at 0ms - Levit 30ms.sub", enable=False)
    prg.add(304759100, "Picture - Field off at 0ms -SternGerlach_Levit 10ms.sub", enable=False)
    prg.add(304759100, "Picture - Field off at 0ms -SternGerlach_Ele.sub", enable=False)
    prg.add(304759100, "Picture - Field off at 0ms -SternGerlach_ShortTom15ms.sub", enable=False)
    prg.add(304759100, "Picture - Field off at 0ms - Levit 50ms.sub", enable=False)
    prg.add(304759100, "Picture - Field off at 0ms -SternGerlach_ShortTom.sub", enable=False)
    prg.add(304759100, "Picture - Field off at 0ms -SternGerlach_ShortElena.sub", enable=False)
    prg.add(304929100, "Picture NaK.sub", enable=False)
    prg.add(304929100, "Picture NaK no Rep.sub", enable=False)
    prg.add(309929100, "Bx Grad OFF")
    prg.add(321702499, "Bias field initialization")
    prg.add(322553199, "Pulse uw OFF")
    prg.add(330787699, "Set MOT NaK.sub")
    prg.add(331287699, "Dark Spot MOT load.sub")
    prg.add(331387699, "Config MOT.sub")
    prg.add(331387699, "Dipole Trap y DAC V", 0.0000)
    return prg
def commands(cmd):
