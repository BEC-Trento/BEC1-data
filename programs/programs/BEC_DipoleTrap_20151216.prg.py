...
def program(prg, cmd):
    ...
    prg.add(129010040, "B comp x", 405.0)
    prg.add(129950000, "Dipole Trap x DAC V", 4.0000)
    prg.add(130000000, "Evaporation Ramp.sub")
    prg.add(405020000, "Dipole Trap x ramp", start_t=0.0000, stop_x=4.000, n_points=300, start_x=0.000, stop_t=100.0000, enable=False)
    ...
    prg.add(682560000, "Picture - Field off at 0ms - Levit 150ms.sub", enable=False)
    prg.add(682560000, "Picture - Field off at 0ms - Levit 50ms.sub", enable=False)
    prg.add(682560000, "Config Field OFF.sub", enable=False)
    prg.add(682560000, "Delta 1 Current ramp", start_t=0.0000, stop_x=0.000, n_points=300, start_x=100.000, stop_t=500.0000, enable=False)
    prg.add(687570000, "Picture NaK.sub", enable=False)
    prg.add(691600000, "Dipole Trap x DAC V", 0.0000)
    prg.add(701150000, "Set MOT NaK.sub")
    prg.add(701650000, "Dark Spot MOT load.sub")
    prg.add(701750000, "Config MOT.sub")
    return prg
