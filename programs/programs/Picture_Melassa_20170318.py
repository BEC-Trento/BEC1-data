...
def program(prg, cmd):
    ...
    prg.add(700999999, "Picture NaK.sub")
    prg.add(714439999, "Config Field OFF.sub", enable=False)
    prg.add(714449999, "Picture NaK.sub", enable=False)
    prg.add(745299999, "Config Field OFF.sub")
    prg.add(745300999, "Picture - Field off at 0ms - Levit 50ms.sub", enable=False)
    prg.add(745300999, "Picture - Field off at 0ms - Levit 180ms.sub", enable=False)
    prg.add(745300999, "Picture NaK Hamamatsu.sub", enable=False)
    prg.add(745300999, "Pulse uw ON", enable=False)
    prg.add(745301099, "Pulse uw OFF", enable=False)
    prg.add(745302099, "Picture NaK no Rep.sub", enable=False)
    prg.add(745302099, "Picture NaK.sub", enable=False)
    prg.add(749072598, "Set MOT NaK.sub")
    prg.add(749572598, "Dark Spot MOT load.sub")
    prg.add(749672598, "Config MOT.sub")
    return prg
def commands(cmd):
