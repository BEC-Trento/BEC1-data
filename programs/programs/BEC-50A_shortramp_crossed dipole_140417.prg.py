...
def program(prg, cmd):
    ...
    prg.add(488285600, "Bx Grad Pulse")
    prg.add(488293600, "Dipole Trap x DAC V", 0.0000, functions=dict(time=lambda x: x+cmd.get_var('dt')))
    prg.add(488293700, "Dipole Trap y DAC V", 0.0000, functions=dict(time=lambda x: x+cmd.get_var('dt')))
    prg.add(488293800, "TTL2-10 OFF", functions=dict(time=lambda x: x+cmd.get_var('dt')))
    prg.add(488293900, "Picture NaK.sub", enable=False)
    prg.add(488293900, "Picture - Field off at 0ms -SternGerlach_Levit 10ms.sub", enable=False)
    prg.add(488293900, "Picture - Field off at 0ms -SternGerlach_Ele.sub", functions=dict(time=lambda x: x+cmd.get_var('dt')))
    prg.add(488293900, "Picture - Field off at 0ms -SternGerlach_ShortElena.sub", enable=False)
    prg.add(488293900, "Picture - Field off at 0ms - Levit 50ms.sub", enable=False)
    prg.add(488293900, "Picture - Field off at 0ms -SternGerlach_ShortElena2017.sub", functions=dict(time=lambda x: x+cmd.get_var('dt'), funct_enable=False), enable=False)
    prg.add(488463900, "Picture NaK.sub", enable=False)
    prg.add(488463900, "Picture NaK no Rep.sub", enable=False)
    prg.add(494473900, "Bx Grad OFF", functions=dict(time=lambda x: x+cmd.get_var('dt'), funct_enable=False))
    prg.add(494474000, "TTL2 5 OFF", enable=False)
    prg.add(506247399, "Bias field initialization", functions=dict(time=lambda x: x+cmd.get_var('dt'), funct_enable=False))
    prg.add(507098097, "Pulse uw OFF", functions=dict(time=lambda x: x+cmd.get_var('dt'), funct_enable=False))
    prg.add(515332598, "Set MOT NaK.sub", functions=dict(time=lambda x: x+cmd.get_var('dt'), funct_enable=False))
    prg.add(515832598, "Dark Spot MOT load.sub", functions=dict(time=lambda x: x+cmd.get_var('dt'), funct_enable=False))
    prg.add(515932598, "Config MOT.sub", functions=dict(time=lambda x: x+cmd.get_var('dt'), funct_enable=False))
    return prg
def commands(cmd):
    import numpy as np
    iters = np.arange(20, 130, 7)
    j = 0
    while(cmd.running):
        dt1 = iters[j]
        cmd.set_var('dt', dt1)
