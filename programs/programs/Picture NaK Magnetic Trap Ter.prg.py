prg_comment = ""
prg_version = "0.5.1"
def program(prg, cmd):
    prg.add(10, "Initialize 0 TTL0")
    prg.add(500, "Config Field OFF.sub")
    prg.add(10000, "Initialize 0 TTL3")
    prg.add(12580, "B comp x", 0.000000)
    prg.add(1500000, "Set MOT.sub", enable=False)
    prg.add(1500000, "Set MOT NaK.sub")
    prg.add(2000000, "Dark Spot MOT load.sub")
    prg.add(2100000, "Config MOT.sub")
    prg.add(5000000, "Synchronize.sub")
    prg.add(70000000, "Rele 5 Close")
    prg.add(72000000, "Na 3D MOT cool (-) Amp", 450.000000)
    prg.add(79500000, "Na 3D MOT cool (-) Amp", 1000.000000)
    prg.add(79950500, "Melassa Na.sub")
    prg.add(79951500, "Melassa Na amp.sub")
    prg.add(79952000, "Config Field OFF.sub")
    prg.add(80000000, "MOT lights Off.sub", enable=False)
    prg.add(80000000, "MOT lights Off NaK.sub")
    prg.add(80004000, "Delta 1 Current", 200.000000)
    prg.add(80004010, "Delta 2 Voltage", 30.000000)
    prg.add(80004020, "Config MT compr.sub")
    prg.add(81110000, "Rele 5 Open")
    prg.add(82003000, "All Shutter Close.sub", enable=False)
    prg.add(82003000, "All Shutter Close NaK.sub")
    prg.add(85000000, "Mirrors Imaging")
    prg.add(86000000, "All AOM On.sub", enable=False)
    prg.add(86000000, "All AOM On NaK.sub", enable=False)
    prg.add(89000000, "B comp x", 660.000000)
    prg.add(90000000, "Config Field OFF ramp.sub")
    prg.add(90000200, "Evaporation Ramp.sub", enable=False)
    prg.add(90050000, "Picture NaK.sub")
    prg.add(375010000, "Decompress Voltage MT.sub")
    prg.add(475940000, "Picture - Field off at 0ms - Levit 10ms - LS.sub", enable=False)
    prg.add(480010000, "Picture - Field off at 0ms - Levit 200ms.sub", enable=False)
    prg.add(480010000, "Picture - Field off at 0ms - Levit 5ms.sub", enable=False)
    prg.add(480010000, "Picture - Field off at 0ms - Levit 30ms.sub", enable=False)
    prg.add(480010000, "Picture - Field off at 0ms - Levit 250ms.sub", enable=False)
    prg.add(480010000, "Picture - Field off at 0ms - Levit 120ms.sub", enable=False)
    prg.add(482580000, "Picture - Field off at 0ms - Levit 10ms.sub", enable=False)
    prg.add(487940000, "Picture - Field off at 0ms - Levit 5ms - LS.sub", enable=False)
    prg.add(497350000, "Picture - Field off at 0ms - Levit 180msNew.sub", enable=False)
    prg.add(497350000, "Picture - Field off at 0ms - Levit 50ms.sub", enable=False)
    prg.add(521500000, "Set MOT NaK.sub")
    prg.add(522000000, "Dark Spot MOT load.sub")
    prg.add(522100000, "Config MOT.sub")
    prg.add(530000000, "Shutter Probe Na Open", enable=False)
    return prg
