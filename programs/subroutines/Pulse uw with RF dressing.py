...
def program(prg, cmd):
    ...
    prg.add(2500, "Pulse uw", polarity=1, pulse_t=0.00200, functions=dict(pulse_t=lambda x: cmd.get_var('marconi1_pulsetime_dressing')*1e-3+cmd.get_var('dipole_mix_time')+cmd.get_var('dipole_hold3_time')+0.001, time=lambda x: x+1e-3*cmd.get_var('marconi1_pulsetime')+1e-3*cmd.get_var('marconi1_pulsetime2')+1e-3*cmd.get_var('probe_z_pulsetime_mix')), enable=False)
    prg.add(2500, "Pulse uw ON", functions=dict(time=lambda x: x+1e-3*cmd.get_var('marconi1_pulsetime')+1e-3*cmd.get_var('marconi1_pulsetime2')+1e-3*cmd.get_var('probe_z_pulsetime_mix')))
    prg.add(3000, "Probe y AOM TTL long ", functions=dict(time=lambda x: x+1e-3*cmd.get_var('marconi1_pulsetime')+1e-3*cmd.get_var('marconi1_pulsetime2')))
    return prg
