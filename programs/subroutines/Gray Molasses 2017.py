...
prg_version = "0.7"
def program(prg, cmd):
    prg.add(-100000, "B comp x", 700.0, functions=dict(value=lambda x: cmd.get_var('Bx_GM')))
    prg.add(-99000, "B comp y", 0.0000, functions=dict(value=lambda x: cmd.get_var('By_GM')))
    prg.add(-98000, "B comp z", 0.8500, functions=dict(value=lambda x: cmd.get_var('Bz_GM')))
    prg.add(0, "GM ON")
