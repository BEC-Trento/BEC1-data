...
prg_version = "0.7"
def program(prg, cmd):
    prg.add(-5013000, "Shutter repump Na Open", enable=False)
    ...
    prg.add(-2980000, "Shutter 3DMOT cool Na Open", enable=False)
    ...
    prg.add(-101000, "Na Probe/Push (+) Amp", 0, functions=dict(amplitude=lambda x: cmd.get_var('probe_amp')))
    prg.add(-100500, "Na Probe/Push (-) Amp", 0, functions=dict(amplitude=lambda x: cmd.get_var('probe_amp')))
    ...
    prg.add(3019500, "Trig ON Stingray 1", enable=False)
    prg.add(3022000, "Trig OFF Stingray 1", enable=False)
