...
def program(prg, cmd):
    ...
    prg.add(500, "Pulse uw with RF imaging m1", functions=dict(time=lambda x: x - 1e-3*cmd.get_var('marconi1_pulsetime_m1')))
    ...
    prg.add(4991, "Pulse uw with RF imaging zero", functions=dict(time=lambda x: x - 1e-3*cmd.get_var('marconi1_pulsetime_p1')))
    ...
    prg.add(5500, "Pulse uw with RF imaging p1", functions=dict(time=lambda x: x - 1e-3*cmd.get_var('marconi1_pulsetime_p1')), enable=False)
