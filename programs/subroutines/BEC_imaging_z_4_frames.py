...
def program(prg, cmd):
    ...
    prg.add(-50, "Pulse uw with RF imaging m1", functions=dict(time=lambda x: x - 1e-3*cmd.get_var('marconi1_pulsetime_m1')), enable=False)
    prg.add(-50, "Pulse uw with RF imaging m1to0", functions=dict(time=lambda x: x - 1e-3*cmd.get_var('marconi1_pulsetime_m1')))
    ...
    prg.add(4991, "Pulse uw with RF imaging p1", functions=dict(time=lambda x: x - 1e-3*cmd.get_var('marconi1_pulsetime_p1')), enable=False)
    ...
    prg.add(4991, "Pulse uw with RF imaging p1to0", functions=dict(time=lambda x: x - 1e-3*cmd.get_var('marconi1_pulsetime_p1')))
