...
def program(prg, cmd):
    ...
    prg.add(237824, "Shutter Probe/Push Close", functions=dict(time=lambda x: x+1e-3*cmd.get_var('cam_z1_ExposureTime') ), enable=False)
    prg.add(402390, "Pulse Trig Stingray z", comment="dark", polarity=1, pulse_t=0.10000, functions=dict(time=lambda x: x+1e-3*cmd.get_var('cam_z1_ExposureTime') ), enable=False)
    return prg
