...
def program(prg, cmd):
    ...
    prg.add(3200, "RF_freq", 60, functions=dict(frequency=lambda x: x*1e6, time=lambda x: x+1e-3*cmd.get_var('marconi1_pulsetime')+1e-3*cmd.get_var('marconi1_pulsetime2')+cmd.get_var('dipole_hold3_time')+1e-3*cmd.get_var('marconi1_pulsetime_dressing')+cmd.get_var('mix_time')))
    return prg
