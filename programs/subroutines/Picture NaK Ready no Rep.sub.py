...
def program(prg, cmd):
    ...
    prg.add(0, "Pulse Probe Na", polarity=1, pulse_t=0.01000, functions=dict(pulse_t=lambda x: 1e-3*cmd.get_var('tprobe'), funct_enable=False))
    ...
    prg.add(1000000, "Pulse Probe Na", polarity=1, pulse_t=0.01000, functions=dict(pulse_t=lambda x: 1e-3*cmd.get_var('tprobe'), funct_enable=False))
