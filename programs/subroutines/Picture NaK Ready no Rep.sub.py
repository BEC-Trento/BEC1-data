...
def program(prg, cmd):
    ...
    prg.add(-10, "Pulse Freq sweep Probe Na", polarity=1, pulse_t=0.00300)
    prg.add(0, "Pulse Probe Na", polarity=1, pulse_t=0.00800, functions=dict(pulse_t=lambda x: 1e-3*cmd.get_var('tprobe'), funct_enable=False))
    prg.add(1050, "Trig OFF Stingray 1")
    prg.add(999500, "Trig ON Stingray 1")
    prg.add(999900, "Pulse Freq sweep Probe Na", polarity=1, pulse_t=0.00300)
    prg.add(1000000, "Pulse Probe Na", polarity=1, pulse_t=0.00800, functions=dict(pulse_t=lambda x: 1e-3*cmd.get_var('tprobe'), funct_enable=False))
