...
def program(prg, cmd):
    ...
    prg.add(-37, "Pulse Freq sweep Probe Na", polarity=1, pulse_t=0.10200)
    prg.add(0, "Pulse Probe Na", polarity=1, pulse_t=0.10000, functions=dict(pulse_t=lambda x: 1e-3*cmd.get_var('tprobe'), funct_enable=False), enable=False)
    prg.add(1050, "Trig OFF Stingray 1")
    prg.add(999500, "Trig ON Stingray 1")
    prg.add(999963, "Pulse Freq sweep Probe Na", polarity=1, pulse_t=0.10200)
    prg.add(1000000, "Pulse Probe Na", polarity=1, pulse_t=0.10000, functions=dict(pulse_t=lambda x: 1e-3*cmd.get_var('tprobe'), funct_enable=False), enable=False)
