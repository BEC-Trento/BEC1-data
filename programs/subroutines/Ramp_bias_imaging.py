...
prg_version = "0.7"
def program(prg, cmd):
    prg.add(0, "B comp x ramp", start_t=0, stop_x=0, n_points=110, start_x=0, stop_t=200, functions=dict(start_x=lambda x: cmd.get_var('Bx_GM')+cmd.get_var('Bx_bottom'), stop_x=lambda x: cmd.get_var('Bx_GM')+cmd.get_var('Bx_tof_imaging')))
