...
def program(prg, cmd):
    ...
    prg.add(-1010000, "Shutter Probe Na Open")
    prg.add(-1002000, "TTL Repumper MOT OFF")
    prg.add(-1000000, "Na Probe/Push (+) OFF")
    ...
    prg.add(-56000, "Na Probe/Push (+) Amp", 1000, functions=dict(amplitude=lambda x: cmd.get_var('probe_amp')))
    prg.add(-55000, "Na Probe/Push (+) freq", 115.00, functions=dict(frequency=lambda x: 110+cmd.get_var('probe_det')/2))
    ...
    prg.add(-10000, "Na Probe/Push (+) OFF")
    return prg
