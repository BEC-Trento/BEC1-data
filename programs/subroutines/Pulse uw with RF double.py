...
def program(prg, cmd):
    ...
    prg.add(230, "RF_freq", 110, functions=dict(frequency=lambda x: cmd.get_var('RF_freq_zero')*1e6, time=lambda x: x+1e-3*cmd.get_var('marconi1_pulsetime')))
    ...
    prg.add(3000, "RF_freq", 60, functions=dict(time=lambda x: x+1e-3*cmd.get_var('marconi1_pulsetime')+1e-3*cmd.get_var('marconi1_pulsetime2')+1e-3*cmd.get_var('marconi1_pulsetime_m1')), enable=False)
    prg.add(3700, "RF_amp", 1, functions=dict(time=lambda x: x+1e-3*cmd.get_var('marconi1_pulsetime')+1e-3*cmd.get_var('marconi1_pulsetime_m1')+1e-3*cmd.get_var('marconi1_pulsetime_m1')), enable=False)
    return prg
