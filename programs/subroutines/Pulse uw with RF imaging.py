...
prg_version = "0.7"
def program(prg, cmd):
    prg.add(-1000, "RF_freq", 108, functions=dict(frequency=lambda x: cmd.get_var('RF_freq_m1')*1e6), enable=False)
    prg.add(-1000, "RF_freq", 110, functions=dict(frequency=lambda x: cmd.get_var('RF_freq_p1')*1e6))
