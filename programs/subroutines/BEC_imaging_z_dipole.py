...
prg_version = "0.7"
def program(prg, cmd):
    prg.add(-6209, "Pulse Trig Stingray z", comment="atoms", polarity=1, pulse_t=0.10000, functions=dict(time=lambda x: x - 1e-3*cmd.get_var('marconi1_pulsetime_m1')-20.1))
    ...
    prg.add(-2500, "Pulse uw with RF imaging m1", functions=dict(time=lambda x: x - 1e-3*cmd.get_var('marconi1_pulsetime_m1')))
    prg.add(-2500, "Pulse uw with RF imaging zero", functions=dict(time=lambda x: x - 1e-3*cmd.get_var('marconi1_pulsetime_zero')), enable=False)
    prg.add(701, "Probe z AOM TTL")
    prg.add(3791, "Pulse Trig Stingray z", comment="probe", polarity=1, pulse_t=0.15445, functions=dict(time=lambda x: x- 1e-3*cmd.get_var('marconi1_pulsetime_p1')))
    prg.add(7701, "Probe z AOM TTL")
