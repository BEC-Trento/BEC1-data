...
def program(prg, cmd):
    ...
    prg.add(-56340, "Probe z AOM TTL", enable=False)
    prg.add(-310, "Repumper AOM TTL", enable=False)
    prg.add(-310, "Pulse uw ON", functions=dict(time=lambda x: x - 1e-3*cmd.get_var('marconi1_pulsetime')))
    prg.add(-310, "Pulse uw OFF")
