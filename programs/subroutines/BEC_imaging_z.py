...
def program(prg, cmd):
    ...
    prg.add(-2250, "Pulse Trig Stingray z", comment="atoms", polarity=1, pulse_t=0.15445, functions=dict(time=lambda x: x - 1e-3*cmd.get_var('marconi1_pulsetime_m1')- 1e-3*cmd.get_var('marconi1_pulsetime_p1')))
    prg.add(-800, "Pulse uw with RF imaging p1", functions=dict(time=lambda x: x - 1e-3*cmd.get_var('marconi1_pulsetime_p1')), enable=False)
    prg.add(-800, "Pulse uw with RF imaging m1", functions=dict(time=lambda x: x - 1e-3*cmd.get_var('marconi1_pulsetime_m1')), enable=False)
