...
prg_version = "0.7"
def program(prg, cmd):
    prg.add(-10000, "Config Field OFF.sub", functions=dict(time=lambda x: x + cmd.get_var('tof')))
    ...
    prg.add(2100, "Delta 1 Current", 0.000, functions=dict(value=lambda x: cmd.get_var('Itof')))
    ...
    prg.add(5000000, "B comp y", 0.0000, functions=dict(time=lambda x: x + cmd.get_var('tof')))
    return prg
