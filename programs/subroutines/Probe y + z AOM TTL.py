...
def program(prg, cmd, delay=0):
    ...
    prg.add(24 + delay*1e4, "Pulse Probe y", polarity=1, pulse_t=0.10000, functions=dict(pulse_t=lambda x: 1e-3*cmd.get_var('probe_pulsetime')))
    prg.add(76 + delay*1e4, "Na Probe/Push (-) amp", 0, functions=dict(time=lambda x: x + 1e-3*cmd.get_var('probe_pulsetime')))
