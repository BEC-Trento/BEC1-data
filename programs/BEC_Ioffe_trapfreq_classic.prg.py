...
def program(prg, cmd):
    ...
    prg.add(160000000, "B comp z", 0.0000, functions=dict(value=lambda x: cmd.get_var('Bz_GM'), time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('hold_time')))
    prg.add(160000000, "Config Levitation", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('hold_time')+cmd.get_var('hold_time_2')))
    ...
    prg.add(160050000, "B comp z ramp", start_t=0.0000, stop_x=0.000, n_points=200, start_x=0.000, stop_t=1000.0000, functions=dict(time=lambda x: x +cmd.get_var('evap1_time')+cmd.get_var('Quad_rampdown_time'), start_x=lambda x: cmd.get_var('Bz_GM'), stop_x=lambda x: cmd.get_var('Bz_GM')+cmd.get_var('Bz_offset')))
