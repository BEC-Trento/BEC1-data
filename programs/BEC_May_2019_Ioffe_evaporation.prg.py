...
def program(prg, cmd):
    ...
    prg.add(200040000, "Evaporation ramp", start_t=0.0000, func_args="fstart=4e6, fend=2e6, tau=2", n_points=500, func="(fstart - fend)*exp(-t/tau) + fend", stop_t=5000.0000, functions=dict(func_args=lambda x: 'fstart={}, fend={}, tau={}'.format(cmd.get_var('evap2_fstart')*1e6, cmd.get_var('evap2_fend')*1e6, cmd.get_var('evap2_tau'))
), enable=False)
    prg.add(205000000, "Ramp_bias_down.sub")
    prg.add(207010000, "Quad_RampUP")
...
def commands(cmd):
    ...
    while(cmd.running):
        print('\n-------o-------')
        By_MT1 = iters[j]
        cmd.set_var('By_MT', By_MT1)
        print('\n')
        print('Run #%d/%d, with variables:\nBy_MT = %g\n'%(j+1, len(iters), By_MT1))
