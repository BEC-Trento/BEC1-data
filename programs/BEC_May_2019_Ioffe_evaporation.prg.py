...
def program(prg, cmd):
    ...
    prg.add(210000000, "BEC_microwave_imaging", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('hold_time')+cmd.get_var('tof')), enable=False)
    ...
    prg.add(210000000, "Config Field OFF.sub", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('hold_time')+cmd.get_var('tof')+30), enable=False)
    ...
    prg.add(220010000, "Na Probe/Push (+) freq", 60.00)
    prg.add(220020000, "Na Probe/Push (-) freq", 60.00)
    return prg
def commands(cmd):
    import numpy as np
