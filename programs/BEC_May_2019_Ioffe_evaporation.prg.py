...
def program(prg, cmd):
    ...
    prg.add(160000000, "Config Field OFF.sub", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('hold_time')+cmd.get_var('tof')+30), enable=False)
    prg.add(160010000, "Quad_RampUP")
    prg.add(160011000, "Green Light AOM amp", 0, functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('hold_time')+cmd.get_var('tof')-0.1-1000, funct_enable=False), enable=False)
