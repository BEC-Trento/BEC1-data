...
def program(prg, cmd):
    ...
    prg.add(190000000, "rf_pulse", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')), enable=False)
    ...
    prg.add(190000010, "Single_frame_imaging", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time') + cmd.get_var('rf_pulse_time') +cmd.get_var('tof')), enable=False)
    ...
    prg.add(190005010, "Pulse uw OFF", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time') + cmd.get_var('rf_pulse_time') +cmd.get_var('tof')), enable=False)
    prg.add(190006010, "Config Levitation", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time') + cmd.get_var('rf_pulse_time') +cmd.get_var('tof')))
    prg.add(190006010, "Config Field OFF.sub", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+ cmd.get_var('rf_pulse_time')+cmd.get_var('tof')+cmd.get_var('tof2')-1))
    prg.add(190006010, "BEC_imaging", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time') + cmd.get_var('rf_pulse_time') +cmd.get_var('tof')+cmd.get_var('tof2')))
    prg.add(195006010, "BEC_imaging_z", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('tof')), enable=False)
    prg.add(205006010, "Config Field OFF.sub", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')), enable=False)
    prg.add(215006010, "DarkSpotMOT_19.sub", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('hold_time')+cmd.get_var('tof')), enable=False)
    prg.add(215306010, "open_probe", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('hold_time')+cmd.get_var('tof')), enable=False)
    return prg
def commands(cmd):
    import numpy as np
    ...
    while(cmd.running):
        print('\n-------o-------')
        rf_pulse_time = iters[j]
        cmd.set_var('rf_pulse_time', rf_pulse_time)
        print('\n')
        print('Run #%d/%d, with variables:\nrf_pulse_time = %g\n'%(j+1, len(iters), rf_pulse_time))
