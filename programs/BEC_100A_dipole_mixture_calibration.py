...
def program(prg, cmd):
    ...
    prg.add(240002468, "Pulse uw with RF double", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('end_of_ODT_transfer')+1))
    ...
    prg.add(240002599, "Dipole trap xy STANDBY", functions=dict(time=lambda x: x+cmd.get_var('evap1_time')+cmd.get_var('evap2_time')+cmd.get_var('end_of_ODT_transfer')+cmd.get_var('dipole_hold_after')))
